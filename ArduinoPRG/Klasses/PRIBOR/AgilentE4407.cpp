...
AnalizE4407::AnalizE4407(int Adr, char* Ip,int LAN)
{
	Control(Adr, 30, Gadr);
	strcpy(GPIB,"24");
	itoa(Gadr, GPIB, 10);
	strcpy(ipAdres, Ip);
	strcpy(ipLok, "10.29.31.26");
	CentrF = 140;
	RL = 20;
	Span = 100;
	RBW = 3000;
...
AnalizE4407::~AnalizE4407()
{
}

char* AnalizE4407::PriborLoad()
{
	 char sttr[100]; //
	   switch (Lan_GPIB)
	   {
...
void AnalizE4407::GPIB_Set(AnsiString Adr, AnsiString Ip)
...
void AnalizE4407::IP_Set(AnsiString Ip)
{      /* TODO -cIP ADR : добавить в метод IP */
	Lan_GPIB = 0;
	strcpy(ipLok, Ip.c_str());
}
char* AnalizE4407::GetGPIB()
{
	return GPIB;
}
char* AnalizE4407::GetIPlok()
{
	return ipLok;
}

AnsiString  AnalizE4407::Zapt(AnsiString str)
{   AnsiString string;
...
long AnalizE4407::AnSpanSet(double Fset, double SP, double BW, double RWFL) {
 AnsiString string;
 //SetMarker(0,0,0,0);        /* TODO -o1 -cМаркеры :  */
...
double AnalizE4407::Sens(double Lin,double Fin, double SP, double BW,double Ofs){
...
int AnalizE4407::Control(int K, int Pred, int& Com) {
	if (K >= 0 && K <= Pred) {
...
long AnalizE4407::MkrPeak(int n) {
	AnsiString string;
	status = viOpenDefaultRM(&defaultRM);
	strcpy(buffer, PriborLoad());
	status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
	...
	status = viPrintf(vi,"INIT:IMM;*WAI\n");
...
long AnalizE4407::MkDelta(int num,bool Enabl,double Del) {
	AnsiString string;
...
long AnalizE4407::MarkerCent(int N){
 	AnsiString string;
    status = viOpenDefaultRM(&defaultRM);
	strcpy(buffer, PriborLoad());
	status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
long AnalizE4407::SetMarker(bool Del, int Delta, bool Pik, int Count) {
	AnsiString string;
    status = viOpenDefaultRM(&defaultRM);
	strcpy(buffer, PriborLoad());
	status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
double AnalizE4407::FMark(){

 return Fmark;
}
char* AnalizE4407::Result(int n) {
		 AnsiString string;
		status = viOpenDefaultRM(&defaultRM);
		strcpy(buffer, PriborLoad());
		status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
char* AnalizE4407::CHPWRBW(double Chnl,double ChBW){
		Pinitial();
...
void AnalizE4407::DataRet(double *Data, double *Freq, int point)
...
long  AnalizE4407::Spurs(int Line)
...
double AnalizE4407::RazborStr (int poz, char* str)
{     char* St;
...
void AnalizE4407::DispLINE(int LEV,int ENbl)
{   status = viOpenDefaultRM(&defaultRM);//Vstende5071->PriborLoad()
	strcpy(buffer,PriborLoad());
	printf_s("%s\n", buffer);
	status = viOpen(defaultRM,(ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
void AnalizE4407::Wite(int Msec)
{
		lOpc=0;
		viPrintf(vi, "*OPC\n");
		Coun = 0;
...
long AnalizE4407::SavPath(AnsiString MPath)
{   DataPath = MPath+"\\";
//	status = viOpenDefaultRM(&defaultRM);
//	strcpy(buffer, PriborLoad());
//	status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
long AnalizE4407::Screen(AnsiString FName)
{
	status = viOpenDefaultRM(&defaultRM);
	strcpy(buffer, PriborLoad());
	status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
