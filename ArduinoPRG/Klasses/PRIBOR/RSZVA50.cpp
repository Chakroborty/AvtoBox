...
PanZVA50::PanZVA50(int Adr, char* Ip) {
	Control(Adr, 30, Gadr);
	GPIB = "21";
	itoa(Gadr, GPIB, 10);
	strcpy(ipAdres, Ip);
...
void PanZVA50::IP_Set(AnsiString Ip)
{      /* TODO -cIP ADR : добавить в метод IP */
	Lan_GPIB = 0;
	strcpy(ipLok, Ip.c_str());
}
void PanZVA50::GPIB_Set(AnsiString Adr, AnsiString Ip) {      /* TODO -cIP ADR : добавить в метод IP */
...
AnsiString  PanZVA50::Zapt(AnsiString str)
{   AnsiString string;
...
long PanZVA50::AnSpanSet(double Fset, double SP, double BW, double RWFL) {
	AnsiString string;
...
char* PanZVA50::GetGPIB() {
	return GPIB;
}
int PanZVA50::Control(int K, int Pred, int& Com) {
	if (K >= 0 && K <= Pred) {
		if (K != 0) {
			Com = K;
		}
...
long PanZVA50::MkDelta(double Del) {
	AnsiString string;
...
long PanZVA50::SetMarker(bool Del, int Delta, bool Pik, int Count) {
	AnsiString string;
...
double PanZVA50::FMark(){

 return Fmark;
}
char* PanZVA50::Result(int n) {
		 AnsiString string;
		status = viOpenDefaultRM(&defaultRM);
		strcpy(buffer, PriborLoad());
		status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
char* PanZVA50::CHPWRBW(double Chnl){
		AnsiString string;
		status = viOpenDefaultRM(&defaultRM);
		strcpy(buffer, PriborLoad());
		status = viOpen(defaultRM, (ViBuf)buffer, VI_NULL, VI_NULL, &vi);
...
void PanZVA50::DataRet(double *Data, double *Freq, int point)
{
		 ...
		 if (status < VI_SUCCESS)
	 {
		...
		int retCount = point;
		strcpy(buffer,"SWE:POIN ");
		Slovo = IntToStr(point);
		strcat(buffer,Slovo.c_str());
        viPrintf(vi, buffer);
		strcpy(buffer,":INITiate1:IMMediate;*WAI");
		viPrintf(vi, buffer);
		//viQueryf(vi, "TRAC:STIM? CH1DATA\n", "%d", &retCount, Freq);	//SENS1:FREQ:DATA?
		viQueryf(vi, ":SENS1:OFFS:PORT1:DATA?\n", "%d", &retCount, Freq);	//SENS1:FREQ:DATA?
		//retCount = 2*retCount;
		viQueryf(vi,":TRACe? CH1Data\n","%,#lf", &retCount, Data);
		viClose(vi);                    // closes session
		viClose(defaultRM);             // closes default session
}
