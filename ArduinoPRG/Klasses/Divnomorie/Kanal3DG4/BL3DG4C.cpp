...
DWORDLONG __fastcall  BL3DG4::SendDATA_SPI_USB(DWORD DATH,DWORD DATL)
...
void __fastcall BL3DG4::DeviceConnected(TRpu16 *newDevice)
{
...
int iNumber = DeviceManager->Devices->IndexOf(newDevice);
	...
	Form1->StatusBar1->Panels->Items[0]->Text = "3Дж4 " + IntToStr((int)Form1->Stend->VstendeBL3DG4->ReadBlokSN());

}
//---------------------------------------------------------------------------

void __fastcall BL3DG4::DeviceRemoved(TRpu16 *oldDevice)
{
...
void BL3DG4::KanalInitial10()
{
			if (Modul3DG10->Modul_All) {
...
void BL3DG4::FriqSET10(double Fin1,bool Ampl,int DB1,int DB2,int Kanal)
	{
					switch(blTupe)
		{
			...
			case 1:
				...
				{
				  case 3:
				 Modul3DG10->SetFriq(6,1,Fin1, DB1, DB2);
				 ModulFCHG->AdrSet(7);break;//	void FriqSET(double Fin1,bool Ampl,int DB1,int DB2,int Kanal);
				  case 4:
				 Modul3DG10->SetFriq(6,2,Fin1, DB1, DB2);
				 ModulFCHG->AdrSet(6);break;
				  default :
				 Modul3DG10->SetFriq(6,1,Fin1, DB1, DB2);
				 ModulFCHG->AdrSet(7); break;
				}

			break;
		}
	   //	ShowMessage("МФЧГ");
		MFCHG_Fset3(Modul3DG10->MKanalDG10->FrGet1,1);
	   //	ShowMessage("Прогрузка канала" + IntToStr(Kanal));
		KanalInitial10();
...
double BL3DG4::LOADGET1()
{
	return Modul3DG10->MKanalDG10->FrGet1;
}
//**************************************FriqSET 3ДЖ2******************
void BL3DG4::FriqSET(double Fin1,bool Ampl,int DB1,int DB2,int Kanal)
	{
				   ...
				   switch (i)
	   {
					...
					switch (Kanal)
		{
		  ...
		  case 0:
		 ModulDG2->SeFriq(0,1,Fin1, Ampl, DB1, DB2);break;
		  case 1:
		 ...
		 default : ModulDG2->SeFriq(0,1,Fin1, Ampl, DB1, DB2);break;
		}
...
void BL3DG4::AmplON(bool ON)
 {   ModulDG2->MKanal->AmplifSET(ON);
  ...
  //************************************МФЧ1*******************
void BL3DG4::MFCH1_Fset(double Fg1)
{
               //выбор МФЧ1
}

void BL3DG4::MFCH1_Fset3(double Fg4159,double Fg1, int Fas1,int Fas2)
{
...
void BL3DG4::MFCH1_Fset1(double Fg4159,double Fg1,int Log)
...
void BL3DG4::MFCH1_Fset2(int  Pol)
  {
		   //	ModulFCH1->
...
double BL3DG4::Geterodin1()
  {
	   return ModulFCHG->Fget1;
  }

//*************************** МФЧ1 set ПХ ПСИ**********************
void BL3DG4::MFCH1_FsetPX(double Fg1)
{
...
//*************************** МФЧ1 set ДЖ ПСИ********************** /
void BL3DG4::MFCH1_FsetDG(double Fg1)
...
//*************************** МФЧ2 set ПХ ПСИ**********************
void BL3DG4::MFCH2_FsetPX(double Fg1)
...
//*************************** МФЧ2 set ДЖ настр********************
void BL3DG4::MFCH2_Fset(double Fg1)
...
void BL3DG4::MFCH2_Fset1(int Cur)
{
...
void BL3DG4::MFCH2_Fset2(int Cur)
  {
		  if(Cur>15)
		  {
...
void BL3DG4::MFCH2_Fset3(double Fg1, int Fas1,int Fas2)
  {
...
//*************************** МФЧ2 set ДЖ ПСИ**********************
void BL3DG4::MFCH2_FsetDG(double Fg1,int Fas1)
  {     ModulFCH2->SetFreq(Fg1,1,false);    //UPR=1 синхр внутр
		SendDATA_SPI_USB(ModulFCH2->Kluch(0),0);
...
/**************************МФЧГ set ДЖ настр***********************/
void BL3DG4::MFCHG_FsetINDEX(double Fg4159)
{
	if(Fg4159<10000) {
		ModulFCHG->SetFreq(Fg4159*2,12,1,1);
	}
	else {
		...
		SendDATA_SPI_USB(ModulFCHG->Kluch(1),0);         //   |                                         4               (4)      //4
		SendDATA_SPI_USB(ModulFCHG->Kluch(2),0);         //
		SendDATA_SPI_USB(ModulFCHG->Kluch(3),0);         //

			for (i = 7; i < 11; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(1),ModulFCHG->SintLOAD_L(1,i)); //МФЧГ ADF4159_1    11              (3)      //7,9,10

		}

		 for (i = 7; i < 11; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(4),ModulFCHG->SintLOAD_L(4,i)); //МФЧГ ADF4159_2     11             (3)      //7,9,10
		}


				for (i = 4; i < 10; i++)
		{
			 SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(3),ModulFCHG->SintLOAD_L(3,i)); //HMC830                                    //4-9
			 Sleep(100);
		}
				for (i = 18; i < 20; i++)
		{
			 SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(3),ModulFCHG->SintLOAD_L(3,i)); //HMC830             20            (7)      //18,19
			 Sleep(100);
		}
//			  for (i = 10; i < 18; i++)
//		{
//			 SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(3),ModulFCHG->SintLOAD_L(3,i)); //HMC830            46            (17)
//			 Sleep(100);
//		}



/*=====  End of Переключение на ADF4002_1  ======*/
//		   ShowMessage("Переключение на ADF4002_1");
//		ModulFCHG->Sint1_ADF4002->CurSET(0);

		for (i = 0; i < 4; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(2),ModulFCHG->SintLOAD_L(2,i)); //ADF4002             4                      // 0-4
		...
		SendDATA_SPI_USB(ModulFCHG->Kluch(1),0);                                     //                   1                     //1

/*==============================
=            Переключение на ADF4002_2            =
==============================*/
//		  ShowMessage("Переключение на ADF4002_2");
//	   ModulFCHG->Sint2_ADF4002->CurSET(0);
			for (i = 0; i < 4; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(5),ModulFCHG->SintLOAD_L(5,i)); //ADF4002_2           4                     // 0-4
		...
		SendDATA_SPI_USB(ModulFCHG->Kluch(2),0);                                        //                1          56  (27)   //1



}

void BL3DG4::MFCHG_Fset3(double Fg4159, int Inic)
{
	if(Fg4159<10000) {
		ModulFCHG->SetFreq(Fg4159*2,12,1,1);
	}
		...
		ModulFCHG->SetFreq(Fg4159,12,0,1);
			...
			//ShowMessage("Выключение ADF4002_2");
			ModulFCHG->Sint2_ADF4002->CurSET(8);
//
			for (i = 0; i < 4; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(5),ModulFCHG->SintLOAD_L(5,i)); //ADF4002_2           4                     // 0-4
...
//		ModulFCHG->Sint1_ADF4002->CurSET(8);
//
//		for (i = 0; i < 4; i++)
//		{
//			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(2),ModulFCHG->SintLOAD_L(2,i)); //ADF4002             4                      // 0-4
...
//		}
			  ...
			  for (i = 10; i < 18; i++)
		{
			 ...
			 Sleep(100);
							...
							switch (Inic) 
		{
			case 0://ShowMessage("Загрузка  ADF44159_1");
							for (i = 8; i < 11; i++)
				{
					SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(1),ModulFCHG->SintLOAD_L(1,i)); //МФЧГ ADF4159_1    11              (3)      //7,9,10
	                SendDATA_SPI_USB(ModulFCHG->Kluch(1),0);
				}
				  //ShowMessage("Загрузка  ADF44159_2");
							for (i = 8; i < 11; i++)
				{
					SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(4),ModulFCHG->SintLOAD_L(4,i)); //МФЧГ ADF4159_2     11             (3)      //7,9,10
					SendDATA_SPI_USB(ModulFCHG->Kluch(2),0);
			...
			break;
			case 1://ShowMessage("Загрузка  ADF44159_1 0-6");
							for (i = 0; i < 6; i++)
				{
					SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(1),ModulFCHG->SintLOAD_L(1,i)); //МФЧГ ADF4159_1    11              (3)      //7,9,10
					SendDATA_SPI_USB(ModulFCHG->Kluch(2),0);
				}
				//ShowMessage("Загрузка  ADF44159_1 6-11");
							for (i = 6; i < 11; i++)
				{
					SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(1),ModulFCHG->SintLOAD_L(1,i)); //МФЧГ ADF4159_1    11              (3)      //7,9,10
					SendDATA_SPI_USB(ModulFCHG->Kluch(2),0);
				...
				//ShowMessage("Загрузка  ADF44159_2 0-6");
							for (i = 0; i < 6; i++)
				{
					SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(4),ModulFCHG->SintLOAD_L(4,i)); //МФЧГ ADF4159_2     11             (3)      //7,9,10
					SendDATA_SPI_USB(ModulFCHG->Kluch(1),0);
				}
				//ShowMessage("Загрузка  ADF44159_2 6-11");
							for (i = 6; i < 11; i++)
				{
					SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(4),ModulFCHG->SintLOAD_L(4,i)); //МФЧГ ADF4159_2     11             (3)      //7,9,10
					SendDATA_SPI_USB(ModulFCHG->Kluch(1),0);
		...
		SendDATA_SPI_USB(ModulFCHG->Kluch(2),0);
	       //
/*=====  End of Переключение на ADF4002_1  ======*/
		//ShowMessage("Прогрузка ADF4002_1");
		ModulFCHG->Sint1_ADF4002->CurSET(3);
		for (i = 0; i < 4; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(2),ModulFCHG->SintLOAD_L(2,i)); //ADF4002             4                           // 0-4
		}
...
=            Переключение на ADF4002_2            =
==============================*/
		//ShowMessage("Прогрузка ADF4002_2");
		ModulFCHG->Sint2_ADF4002->CurSET(1);
			for (i = 0; i < 4; i++)
		{
			SendDATA_SPI_USB( ModulFCHG->SintLOAD_H(5),ModulFCHG->SintLOAD_L(5,i)); //ADF4002_2           4                            // 0-4
		}
...
void BL3DG4::MFCHG_Fset2(int Cur)
  {
		  if(Cur>15)
		  {
...
void BL3DG4::MFCHG_Fset12(int Cur,int Pol)
  {
...
void BL3DG4::MFCHG_Fset5(int Cur,int Pol)
  {
...
void BL3DG4::MFCHG_OGvnutr(int OG)
...
void BL3DG4::MFCHG_Sinc(double Fg1)
{
		for (i = 0; i < 4; i++)
	{
...
/************************ Синхронизация МФЧГ ***************/
void BL3DG4::MFCH2_Sinc(double Fg1)
{
		for (i = 0; i < 4; i++)
	{
		SendDATA_SPI_USB( ModulFCH2->SintLOAD_H(4),ModulFCH2->SintLOAD_L(4,i)); //ADF4002
...
void BL3DG4::MFCH2_OGvnutr(bool OG)
{
...
void BL3DG4::MFCH2DG_OGvnutr(bool OG)
{   ModulFCH2->OGvnutr(OG);
...
double BL3DG4::FriqOUT(void)
{
	return Modul3DG10->MKanalDG10->FreqFout();
}

double BL3DG4::Geterodin2()
{
	return 2300;
}
 //************************************Blok********************

int BL3DG4::ReadSN_Ver(int SelectedModul)
{
  if(DeviceUSB==NULL)
  {
	   return 0;
...
BYTE __fastcall BL3DG4::SPI_ReadBytePowerEeprom(BYTE ModulADr,WORD Addr,bool by_way)
{
   BYTE AddrLo=Addr;
   BYTE AddrHi=Addr>>8;
   DWORD DWtx=0;
...
BYTE __fastcall BL3DG4::SPI_I2C_WaitRdy(BYTE ModulADr,bool by_way)
	{

	  //Sleep(10);//вместо цикла ожидания, пока
	  return (BYTE)SPI_I2C_GetStatus(ModulADr,by_way);
	}
BYTE __fastcall BL3DG4::SPI_I2C_GetStatus(BYTE ModulADr,bool by_way)
	 {
...
BYTE BL3DG4::SPI_ReadBytePower(BYTE ModulADr,BYTE Addr,bool by_way)
	{
	 return (BYTE)SPI_I2C_rdByte(ModulADr,0x14,Addr,by_way);
	}
void BL3DG4::SPI_WriteBytePower(BYTE ModulADr,BYTE Addr, BYTE Data,bool by_way)
	{
	 SPI_I2C_wrByte(ModulADr,0x14,Data,Addr,by_way);
	}

BYTE __fastcall BL3DG4::SPI_I2C_rdByte(BYTE ModulADr,BYTE CMD,BYTE adr,bool by_way)
	{
...
//---------------------------------------------------------------------------
bool __fastcall BL3DG4::SPI_I2C_wrByte(BYTE ModulADr,BYTE CMD,BYTE D,BYTE adr,bool by_way)
{
...
void __fastcall BL3DG4::SPI_WriteBytePowerEeprom(BYTE ModulADr,BYTE D,WORD Addr,bool by_way)
{
...
void __fastcall BL3DG4::SPI_ErasePageWriteBytePowerEeprom(BYTE ModulADr,WORD Addr, BYTE Data,bool by_way)
{
 BYTE CMD=0x14;
 BYTE AddrHi=Addr>>8;
 BYTE AddrLo=Addr;
...
void __fastcall BL3DG4::SPI_EraseBlockPowerEeprom(BYTE ModulADr,WORD Addr,bool by_way)
{
...
void __fastcall BL3DG4::SPI_WritePagePowerEeprom(BYTE ModulADr,WORD Addr, PBYTE Data, BYTE Size,bool by_way)
{
...
void __fastcall BL3DG4::WriteSN_Ver(int SelectedModul,DWORD ver,DWORD num)
{
if(DeviceUSB==NULL)
  {
...
void __fastcall BL3DG4::SPI_InitI2c()
{
