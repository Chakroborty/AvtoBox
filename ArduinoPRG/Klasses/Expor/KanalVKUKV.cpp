...
#include "KanalVKUKV.h"
#include <math.h>
#include "UsbDeviceManager.h"
//---------------------------------------------------------------------------
 KanalVKUKV::KanalVKUKV(int Mod_kanalNum, int Blok_kanalNum, double Polosa)
 ...
 KanalVKUKV::~KanalVKUKV()
	{
		//Freal = 10;
	}
 void KanalVKUKV::AmplifSET(bool Amplif)
 {   AmpSET = Amplif;
	 if (FregIN>=99)
	 {
		 if (Amplif)
 ...
 void KanalVKUKV::WriteFregIN(double freqCR, int Kanal)
{      if (Kanal==1||Kanal==2)
	{
	...
	} else
	   {
		   ...
		   /* TODO -oУстановка частоты -c1 : Шагом 0,5 */
		 Freal = freqCR;
//		 FregIN = floor(0.5 + freqCR/0.5);
//		 FregIN =  FregIN * 0.5;
		 FregIN = 0.00001*floor(freqCR*100000);


		if (Freal>=100)
		{   AMP_SW3_Bolshe105 = KluchAmplSW3::SW3_105_3200;
			PC1_Kluch = KluchPC1::PC1_105_1215; // Добавлено посл кор
				FsSperenosom();
		}
		else {

			AMP_SW3_Bolshe105 = KluchAmplSW3::SW3_32_105;  //Ключ Усилитель
			PC1_Kluch = KluchPC1::PC1_32_105; // Добавлено посл кор
	 ...
	 KanalVKUKV::WriteFrPC1(FsigPC1);

}
 void KanalVKUKV::FsBolshe1830()//FsBolshe1830PR
		 {
		  Fpc1 = 735;
 ...
 void KanalVKUKV::FsMenshe1215()//FsMenshe1215PR
		 {
		 Fpc1 = 2440;
		 FrGet1 = FregIN + Fpc1;//2 * floor(0.5 + (FregIN + Fpc1) / 2);
		 FsigPC1 =  FrGet1 - Freal;
		 //PC1_Kluch = KluchPC1::PC1_105_1215;
				...
				{Pres_UPR_Kluch =  KluchPrec::Pr_105_175;
					// if (PolosaPC2==70)
					// {
					//  PC1_Kluch = KluchPC1::PC1_105_175_OB;       //Ключ ПЧ1
					//  PC2_Kluch = KluchPC2::PC2_105_175_OB;       //Обход ПЧ1
					//  FsigPC1 = Freal;
					// }
				}
				if (Freal <= 50)
 ...
 void KanalVKUKV::WriteFrPC1(double freqPC1)//WriteFrPC1PR
 {     FsigPC1 = freqPC1;
 ...
 void KanalVKUKV::FsSperenosom()//FsSperenosomPR
			  {
			  KanalVKUKV::AmplifSET(AmpSET);
...
{    int i;
 ...
 void KanalVKUKV::ATT1_SET(double DB)
{
	/*
	   int i;
		 int  ATT_AT;
 ...
 int KanalVKUKV::LoadATT(int div)
{    int i;
	 int  ATT_db;
 if (div >= 0 && div < 8)
   {
 ...
 void KanalVKUKV::ATT2_SET(double DB)
	   {
   if (DB >= 0 && DB <= 63)
   {
		Pch1_Att1 = (int)(DB) <<14;
...
double KanalVKUKV::FreqFout(){
return FsigPC2;
}
//--------------------------------------------------------------
DWORD KanalVKUKV::LoadGET1H(){
  DWORD SW = 0;
	if (true) {
...
//--------------------------------------------------------------
DWORD KanalVKUKV::LoadGET1(int Reg){
  int i;
  DWORD  SW_SW;
 if (Reg >= 0 && Reg < 16)
   {
...
DWORD KanalVKUKV::LoadGET2(int Reg){
  int i;
  DWORD  SW_SW;
 if (Reg >= 0 && Reg < 16)
   {
