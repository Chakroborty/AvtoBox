...
#include "ADF4371.h"
 ...
 ADF4371::ADF4371(double F,int I,int N,double Ref)
 {
	//FregSIN = F;
	R = 1;
	Nref = N;
...
REGIM[0].NINT = 54;		// TPRC51->FCHP2594->PADF4371->NSET(56);//34 36
REGIM[0].CURSET = 4; 	// SENDLAN(TPRC51->FCHLOAD_LMX(TPRC51->FCHP2594->PADF4371->CURSET(4)),0,0);//PDCUR
REGIM[0].SEG1EN = 0; 	// SENDLAN(TPRC51->FCHLOAD_LMX(TPRC51->FCHP2594->PADF4371->SEG1EN(0)),0,0);//SEG
REGIM[0].FCAL_EN = 0;	// SENDLAN(TPRC51->FCHLOAD_LMX(TPRC51->FCHP2594->PADF4371->FCAL_EN(0)),0,0);//FCAL_EN
REGIM[0].OUTAPWR = 63;	// TPRC51->FCHP2594->PADF4371->OUTAPWR(63);//44
REGIM[0].OUTBPWR = 63;	// TPRC51->FCHP2594->PADF4371->OUTBPWR(63);//45
REGIM[0].CHANDiV = 0;	// TPRC51->FCHP2594->PADF4371->CHANDiV(0);//R75[10:6]


// TPRC51->FCHP2594->PADF4371->FRSET(0);//42 43
// TPRC51->FCHP2594->PADF4371->CHANDiV(0);//R75[10:6]
...
//REG[9] = REG[9]|OSC2<<12;
 ...
 void ADF4371::DIVOUTEN(int ENBL)
 {
...
//		Reg050 |=(1<<11); // Чтобы  записат ь  единицу  в  бит  N,   выполни:
 ...
 void ADF4371::SetREFPaTH(int OSC2,int PLL_PRE,int MLT,int PLL_R)
 {
...
void ADF4371::Default()
{
	/******Регистр Set***/
		...
		REG[46]	= 0x2E07FD; // 0x2E07FD; 	0x2E07FD
		REG[45]	= 0x2DC8DF; // 0x2DC8DF; 	0x2DC8DF
		REG[44]	= 0x2C1F23; // 0x2C1FA3; 	0x2C1FA3
...
void ADF4371::SETRGIM(int N,int CUR,int SEG1,int FCAL,int APWR,int BPWR,int CDiV)
{
	NSET(N);
	CURSET(CUR);
	SEG1EN(SEG1);
...
void ADF4371::FReset(int RES)
{
	REG[0] = REG[0]&0xFFFFFD;
	REG[0] = REG[0]|RES<<1;
}
void ADF4371::PDovn(int PD)
{
	REG[0] = REG[0]&0xFFFFFE;
	REG[0] = REG[0]|PD;
}

void ADF4371::MoUTLD(int SEL)
{
	REG[0] = REG[0]&0xFFFFFB;
	REG[0] = REG[0]|SEL<<2;
}

void ADF4371::LDTupe(int Sel)
{
	REG[59] = REG[59]&0xFFFFFE;  //3B 59  1110
	REG[59] = REG[59]|Sel;
}
void ADF4371::LDDLY(int SEL)
{
...
int ADF4371::VCO_SEL_FORCE(int VALUE)
{
	REG[20] = REG[20]&0xFFFBFF;  // R20[10]  FFFBFF
	REG[20] = REG[20]|VALUE<<10;
	return 20;
}
int ADF4371::VCO_SEL(int VALUE)
{
	VCOSEL = VALUE;
...
void ADF4371::FCALHPFDADJ(int VALUE)
{
	REG[0] = REG[0]&0xFFFE7F;
	REG[0] = REG[0]|(VALUE<<7);	//R0[8:7]
}
void ADF4371::FCALLPFDADJ(int VALUE)
{
	REG[0] = REG[0]&0xFFFF9F;
	REG[0] = REG[0]|(VALUE<<5);	//R0[6:5]
}
void ADF4371::CALCLKDIV(int VALUE)
{
...
int ADF4371::CHANDiV(int VALUE)//R75[10:6]
{
	REG[75] = REG[75]&0xFFF83F;
	REG[75] = REG[75]|VALUE<<6;
	return 75;
}
int ADF4371::SEG1EN(int VALUE)//R31[14]
{
	REG[31] = REG[31]&0xFFBFFF;
	REG[31] = REG[31]|VALUE<<14;
		return 31;
}
int ADF4371::OUTAPWR(int VALUE)//R44[13:8]
{
	REG[44] = REG[44]&0xFFC0FF;
	REG[44] = REG[44]|VALUE<<8;
		return 44;
}
int ADF4371::OUTAMUX(int VALUE)//R45[12:11] E 7 FF
{
...
int ADF4371::OUTBPWR(int VALUE)//R45[5:0]
{
...
int ADF4371::OUTBMUX(int VALUE)//R46[1:0]
{
...
void ADF4371::SetFreq(double fsint)
 {   int a = 0;
...
long ADF4371::RegLoadADF4371(int NReg)
  {
...
long ADF4371::RegLoadINIT(int RG)
{
		switch (RG)
	{
...
long ADF4371::RegLoadF(int RG)
{
	//	TODO: Load ADF4371 CR(3)
